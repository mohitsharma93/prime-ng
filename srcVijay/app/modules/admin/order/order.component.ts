...
export class OrderComponent extends BaseComponent implements OnInit {
  ...
  public export(orderStatus: number): void {
    ...
    if (orderStatus === 0 && this.allStatusSelected && this.allStatusSelected.code >= 0) {
      filterQuery = `Status=${orderStatus}&PageNo=1&PageSize=2000&orderStatus=${this.allStatusSelected.code}&searchTimeRange=${dates}&ame=${searchValue}`;
    } else {
      filterQuery = `Status=${orderStatus}&PageNo=1&PageSize=2000&searchTimeRange=${dates}&ame=${searchValue}`;
    }
